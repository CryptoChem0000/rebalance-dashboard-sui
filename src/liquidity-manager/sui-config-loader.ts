...
export const loadSuiConfigWithEnvOverrides = async (
  ...
  envFilePath?: string
): Promise<{ config: SuiConfig; configPath: string }> => {
  // Get config path
  const workingDir = await getWorkingDirectory();
  const configPath = configFilePath ?? path.join(workingDir, "sui-config.json");

  // Try to load default config, but don't fail if it doesn't exist
  let defaultConfig: SuiConfig | null = null;
  ...
  try {
    if (existsSync(configPath)) {
      ...
      defaultConfig = JSON.parse(configContent) as SuiConfig;
  ...
  const config: SuiConfig = {
    ...
    cetusPool: {
      id: finalEnvVars.CETUS_POOL || defaultConfig?.cetusPool?.id || "",
    },

    cetusPosition: {
      id: defaultConfig?.cetusPosition?.id || "",
      bandPercentage: finalEnvVars.CETUS_POSITION_BAND_PERCENTAGE
        ? parseFloat(finalEnvVars.CETUS_POSITION_BAND_PERCENTAGE)
        : defaultConfig?.cetusPosition?.bandPercentage ?? 0,
    },
  };
  ...
  if (!config.cetusPool.id) {
    missingFields.push("CETUS_POOL_ID (cetusPool.id)");
  }
  ...
  if (
    config.cetusPosition.bandPercentage === 0 &&
    !finalEnvVars.CETUS_POSITION_BAND_PERCENTAGE
  ) {
    missingFields.push(
      "CETUS_POSITION_BAND_PERCENTAGE (cetusPosition.bandPercentage)"
    );
  }

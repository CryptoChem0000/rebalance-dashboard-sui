...
program
  ...
  .action(async (options) => {
    ...
    let manager: LiquidityManager | undefined;
    ...
    try {
      ...
      // Initialize manager
      manager = await LiquidityManager.make({
      ...
      gracefulShutdown.registerHandler({
        waitForOperation: async () => {
          // Manager-specific wait logic if needed
        },
        cleanup: async () => {
      ...
      if (options.watch && options.watch > 0) {
        ...
        while (!gracefulShutdown.isShutdownRequested()) {
          ...
          try {
            const watchOperation = manager.execute();
            gracefulShutdown.setCurrentOperation(watchOperation);
            const watchResult = await watchOperation;
            gracefulShutdown.setCurrentOperation(null);
      ...
      async () => {
        try {
          ...
          if (options.type === "all" || options.type === "archway") {
            ...
            console.log(
              dbQueryClient.formatVolumeData(archwayVolume) || "No data"
            );
            console.log();
          }
          ...
          if (options.type === "all" || options.type === "osmosis") {
            ...
            console.log(
              dbQueryClient.formatVolumeData(osmosisVolume) || "No data"
            );
            console.log();
          }
          ...
          if (options.type === "all" || options.type === "bridge") {
            ...
            console.log(
              dbQueryClient.formatVolumeData(bridgeVolume) || "No data"
            );
            console.log();
          }
        ...
        try {
          ...
          transactions.forEach((tx, index) => {
            ...
            if (tx.outputTokenName && tx.outputAmount) {
              console.log(
                `   Output: ${tx.outputAmount} ${tx.outputTokenName}`
              );
            }
      ...
      async () => {
        try {
          ...
          if (options.csv) {
            console.log("\n📁 Exporting to CSV files...");
            const summaryFile =
              await dbQueryClient.exportTransactionSummaryToCSV(
                startDate,
                endDate
              );

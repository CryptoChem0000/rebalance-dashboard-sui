...
import { SuiLiquidityManager } from "../../liquidity-manager";

export function suiCommand(program: Command) {
  program
    .command("sui")
    .description("Run the liquidity management process on SUI network")
    .option(
      "-e, --environment <env>",
...
async function executeWithTracking(
  ...
  try {
    ...
    if (isWatchMode) {
      ...
      return {
        poolId: manager.config.cetusPool.id || "unknown",
        positionId: manager.config.cetusPosition.id || "unknown",
